...
void WdWddW_Spring3(
  ...
  const Eigen::Matrix3d check = stiffness * Eigen::Matrix3d::Identity();
  ddw[0][0] = n + C*(check-n)/length;
  ddw[1][1] = n + C*(check-n)/length;
  ddw[0][1] = -n - C*(check-n)/length;
  ddw[1][0] = -n - C*(check-n)/length;
}
